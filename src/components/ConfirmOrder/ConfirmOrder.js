...
const ConfirmOrder = () => {
  ...
  if (shipInfo.country === "VN") {
    ...
    for (let i = 0; i < products.length; i++) {
      // nếu mà ship free thì ko cộng gi cả
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      if (products[i].internationalShipmentPolicy === "custom") {
        // shippingCharge" sẽ được tính bằng cách nhân số lượng sản phẩm với customLocalShipmentCost
        shippingCharge =
          shippingCharge +
  ...
  return (
    <>
      ...
      <Button
        variant="contained"
        sx={{ m: 4, width: "250px", display: "block", m: "0 auto" }}
