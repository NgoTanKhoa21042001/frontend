...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    ...
    dispatch(getBrands({ id, toast }));
    dispatch(getCategories({ id, toast }));
    dispatch(getStores({ id, toast }));
  ...
  useEffect(() => {
    if (product) {
      ...
      setCustomInternationalShipmentCost(
        product?.setCustomInternationalShipmentCost
      );
        ...
        <Box sx={{ m: "0 auto", marginTop: 2, maxWidth: "550px" }}>
          ...
          <Box component="form" onSubmit={handleSubmit}>
            ...
            <Grid container spacing={2} sx={{ mt: "4px" }}>
              {/*  chiếm 6/12 (hay 50%) của chiều rộng của lưới trên màn hình */}
              <Grid item xs={6}>
